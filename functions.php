...
if (!function_exists('qode_styles')) {
	...
	function qode_styles() {
		...
		if(qode_is_woocommerce_installed()) {
			$style_dynamic_deps_array = array('qode_woocommerce');
		}
...
if (!function_exists('qode_scripts')) {
	...
	function qode_scripts() {
		...
		wp_enqueue_script("lemmonSlider", QODE_ROOT."/js/plugins/lemmon-slider.js",array(),false,true);
		...
		if($qode_options['enable_google_map'] == "yes" || qode_is_ajax_enabled() || qode_has_google_map_shortcode()) {
            ...
            if(isset($qode_options['google_maps_api_key']) != '') {
		...
		wp_localize_script( 'qode_default', 'QodeAdminAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
...
if(!function_exists('qode_browser_specific_scripts')) {
	...
	function qode_browser_specific_scripts() {
		...
		//is ie?
		if ($is_IE) {
			wp_enqueue_script("qode_html5", QODE_ROOT."/js/plugins/html5.js",array(),false,false);
		}
	}
...
if (!function_exists('qode_hide_top_header_on_mobile_class')) {
	function qode_hide_top_header_on_mobile_class($classes) {

        if(qode_options()->getOptionValue('header_top_area_hide_on_mobile') == 'yes'){
	...
	add_filter('body_class','qode_hide_top_header_on_mobile_class');
}
